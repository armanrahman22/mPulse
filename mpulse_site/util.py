...
####################### FILE UPLOAD/VIEW / SERVE ############################

#Returns the filepath used for session file uploads
...
#Returns filepath for log file uploads
def getLogFilePath(instance,filename):
        return 'logFiles/'+str(instance.pk)+'/'+filename
...
def sessionPDF(subjectName,sessionDate,location,sessionData,ecgData):
    #Process ECG data to make it all positive
    if ecgData:
        minVal = min([x[1] for x in ecgData])
        ecgData = [[x[0],x[1]+abs(minVal)] for x in ecgData]
    width,height = letter
    ...
    #Closing paragraph
    if not location:
        message = "Thank you for visiting an M-Pulse Kiosk today. Stay healthy!"
    else:
        message = "Thank you for visiting the M-Pulse Kiosk at "+location+". Stay healthy!"
